...
pastIetfsFile = os.path.join(os.path.dirname(__file__), 'IETF_Past_Meetings.html')
with open(pastIetfsFile) as fp:
  pastIetfsSoup = BeautifulSoup(fp, "lxml")

# Using offline download pulled from this link:
# https://www.ietf.org/about/groups/iesg/past-members/
pastIesgFile = os.path.join(os.path.dirname(__file__), 'IESG_Past_Members.html')
...
CURRENT_IETF = 117
CURRENT_IESG = ['Andrew Alston', 'Roman Danyliw', 'Martin Duke', 'Lars Eggert',
                'Erik Kline', 'Murray Kucherawy', 'Warren Kumari', 'Francesca Palombini',
                'Zaheduzzaman Sarker', 'John Scudder', 'Éric Vyncke', 'Robert Wilton',
                'Paul Wouters', 'Jim Guichard']
...
for ad in adIetfs:
  ietfs = adIetfs[ad]
  adEnds[ad] = [ietfEndDates[i] for i in ietfs if i + 1 not in ietfs and i != CURRENT_IETF]
